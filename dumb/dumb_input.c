...
static void translate_special_chars(char *s)
{
  ...
  while (*src)
	...
	*dest++ = ZC_FKEY_MIN + src[-1] - '0' - 1; break;
      ...
      default:
	fprintf("DUMB-FROTZ: unknown escape char: %c\n", src[-1]);
	fprintf("Enter \\help to see the list\n");
      }
    }
  *dest = '\0';
}
			   ...
			   zchar *continued_line_chars)
{
  ...
  for (;;) {
    ...
    else { dumb_show_prompt(show_cursor, (timeout ? "tTD" : ")>}")[type]); }
    getline(s);
	...
	fprintf("DUMB-FROTZ: No input to discard\n");
      else {
	...
	dumb_dump_screen();
      ...
      fprintf("DUMB-FROTZ: unknown command: %s\n", s);
      fprintf("Enter \\help to see the list of commands\n");
    }
  }
}
