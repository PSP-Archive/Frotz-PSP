...
{
	unsigned char a;
	if(fread(&a, 1, 1, f) != 1) return EOF;
	return (int)a;
}
int put_c(int b, FILE *f)
{
	unsigned char a = (unsigned char)b;
	if(fwrite(&a, 1, 1, f) != 1) return EOF;
	return 1;
}
...
zword restore_quetzal (FILE *svf, FILE *stf)
{
	...
	debugPrintf("restore_quetzal(0x%08X, 0x%08X)\n", svf, stf);
    /* Check it's really an `IFZS' file. */
    debugPrintf("Check it's really an `IFZS' file.\n");
    ...
    debugPrintf("Sanity checks on ifzslen=%d\n", ifzslen);
    if ((ifzslen & 1) || ifzslen<4) /* Sanity checks. */	return 0;
    ifzslen -= 4;

    /* Read each chunk and process it. */
    debugPrintf("Read each chunk and process it.\n");
    while (ifzslen > 0)
    {
	/* Read chunk header. */
	debugPrintf("Read chunk header.\n");
	...
	switch (tmpl)
	{
	    ...
	    case ID_IFhd:debugPrintf("`IFhd' header chunk; must be first in file.\n");
	    ...
	    case ID_Stks:debugPrintf("`Stks' stacks chunk; restoring this is quite complex. ;)\n");
	    ...
	    case ID_CMem:debugPrintf("`CMem' compressed memory chunk; uncompress it.\n");
	    ...
	    case ID_UMem:debugPrintf("`UMem' uncompressed memory chunk; load it.\n");
		if (!(progress & GOT_MEMORY))	/* Don't complain if two. */
		{
	    ...
	    default:debugPrintf("Unrecognised chunk type; skip it.\n");
     ...
     */
     debugPrintf("End of chunk loading\n");
    if (!(progress & GOT_HEADER))
	...
	debugPrintf("restore_quetzal() Returning %d\n", (progress == GOT_ALL ? 2 : fatal));
...
zword save_quetzal (FILE *svf, FILE *stf)
{
    ...
    debugPrintf("Write `Stks' chunk. You are not expected to understand this. ;)\n");
    ...
    debugPrintf("Write out the rest of the stack frames.\n");
    for (i=n; i>0; --i)
    {
    ...
    debugPrintf("Fill in variable chunk lengths.\n");
    ...
    debugPrintf("save_quetzal() return 1\n");
    return 1;
}
